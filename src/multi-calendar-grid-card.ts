/**
 * Multi-Calendar Grid Card — Lit + TypeScript migration (v0.6.0-step5)
 */
...
interface HassEntityEventRaw { start: any; end: any; summary?: string; location?: string; description?: string; [k: string]: any; }
interface NormalizedEvent { s: Date; e: Date; allDay: boolean; summary: string; location?: string; description?: string; color: string; src: string; raw: HassEntityEventRaw; }
...
export interface HomeAssistantLike { callApi(method: string, path: string): Promise<any>; locale?: { language?: string; time_format?: "12" | "24" }; }

const VERSION = "0.6.0-step5";
...
export class MultiCalendarGridCard extends LitElement {
  ...
  public setConfig(config: McgConfig) {
    ...
    this._active = {}; for (const e of this._config.entities) this._active[e.entity] = true;
    try {
  ...
  private async _fetchEvents() {
    ...
    for (let i=0;i<7;i++) {
      const d0 = addMin(start, i*24*60); const d1 = addMin(d0, 24*60);
      const allDay: NormalizedEvent[] = []; const timed: TimedLayout[] = [];
      for (const ev of all) {
        const srcId = (ev as any).__id as string; if (!this._active[srcId]) continue;
        const n = this._norm(ev as any, srcId); if (!(n.e > d0 && n.s < d1)) continue;
        if (n.allDay) { if (this._config.show_all_day) allDay.push(n); }
        else { const startM = Math.max(0, (n.s.getTime()-d0.getTime())/60000); const endM = Math.max(0, (n.e.getTime()-d0.getTime())/60000);
               const top = Math.max(0,startM); const height = Math.max(1, Math.min(1440, Math.max(top+1,endM)) - top);
               timed.push({ n, top, height, lane: 0 }); }
      }
      timed.sort((a,b)=> a.top-b.top || b.height-a.height);
      const lanes:number[]=[]; for (const ev of timed){ let placed=false; for(let li=0; li<lanes.length; li++){ if(ev.top>=lanes[li]){ lanes[li]=ev.top+ev.height; ev.lane=li; placed=true; break; } } if(!placed){ ev.lane=lanes.length; lanes.push(ev.top+ev.height); } }
  ...
  private _norm(ev: HassEntityEventRaw, src: string): NormalizedEvent {
    ...
    const hasTime = Boolean((ev as any).start?.dateTime || (ev as any).end?.dateTime || (typeof sRaw==="string" && sRaw.includes("T")) || (typeof eRaw==="string" && eRaw.includes("T")));
    const s=new Date(sRaw); const e=new Date(eRaw); const allDay=!hasTime;
    return { s,e,allDay, summary:(ev as any).summary||"(no title)", location:(ev as any).location, description:(ev as any).description, color:(ev as any).__color||"#3366cc", src, raw:ev };
  }

  private _startTimers(){ this._stopTimers(); this._tick=window.setInterval(()=> (this as any).requestUpdate(), 60*1000); const mins=clamp(Number(this._config?.data_refresh_minutes)||5,1,60); this._refresh=window.setInterval(()=> this._fetchEvents(), mins*60*1000); }
  private _stopTimers(){ if(this._tick) window.clearInterval(this._tick); if(this._refresh) window.clearInterval(this._refresh); this._tick=undefined; this._refresh=undefined; }

  private _recomputeWeekBounds(){ const base=weekStartOf(new Date(), this._config?.first_day ?? 1); const s=new Date(base); s.setDate(s.getDate()+this._weekOffset*7); this._weekStart=s; }
  private _saveOffset(){ if(!this._config.remember_offset) return; try{ localStorage.setItem(this._config.storage_key!, String(this._weekOffset)); }catch{} }
  private _restoreScroll(){ const minPrefMin=minsFrom(this._config.slot_min_time!); const ppm=Number(this._config.px_per_min)||1.6; const headerOffset=24; const minPrefPx=Math.max(0, Math.round(minPrefMin*ppm)-headerOffset); let target=minPrefPx; if(this._config.remember_offset){ try{ const key=this._config.storage_key!+'.scrollTop'; const saved=Number(localStorage.getItem(key)); if(Number.isFinite(saved)) target=Math.max(minPrefPx, saved); }catch{} } requestAnimationFrame(()=> requestAnimationFrame(()=>{ const sc=(this.shadowRoot||this.renderRoot).querySelector<HTMLElement>(".scroll"); if(sc) sc.scrollTop=target; })); }
  private _persistScroll(){ if(!this._config.remember_offset) return; try{ const sc=(this.shadowRoot||this.renderRoot).querySelector<HTMLElement>(".scroll"); const key=this._config.storage_key!+'.scrollTop'; if(sc) localStorage.setItem(key, String(sc.scrollTop)); }catch{} }
  private _shiftWeek(delta=0,toToday=false){ if(toToday) this._weekOffset=0; else this._weekOffset+=delta; this._saveOffset(); this._recomputeWeekBounds(); this._fetchEvents(); (this as any).updateComplete.then(()=> this._restoreScroll()); }
  private _fmtRange(s:Date,e:Date){ const hassLocale=(this as any)?.hass?.locale; const locale=this._config.locale||hassLocale?.language||"en"; const hourCycle:any=hassLocale?.time_format==="12"?"h12":"h23"; const tFmt=new Intl.DateTimeFormat(locale,{hour:"2-digit",minute:"2-digit",hourCycle}); const dFmt=new Intl.DateTimeFormat(locale,{weekday:"short",day:"2-digit",month:"short"}); const same=s.toDateString()===e.toDateString(); return same?`${dFmt.format(s)} • ${tFmt.format(s)}–${tFmt.format(e)}`:`${dFmt.format(s)} ${tFmt.format(s)} → ${dFmt.format(e)} ${tFmt.format(e)}`; }
  ...
  render(){
    const ws=this._weekStart; const we=addMin(new Date(ws), 7*24*60 - 1);
    ...
    return html`
      <ha-card>
        <div class="hdr">
          <div class="legend">${this._config.entities.map(ent => this._legendItem(ent))}</div>
          <div class="badge" title="${this._error || ""}">${fmt.format(ws)} – ${fmt.format(we)}</div>
          <div class="toolbar">
  ...
  private _legendItem(ent:McgEntityConfig){
    const active=!!this._active[ent.entity]; const cls=active? "item":"item inactive";
  ...
  private _timeColumn(){
    const elems:any[]=[]; const ppm=Number(this._config.px_per_min)||1.6;
    for(let m=0;m<=1440;m+=30){ const y=Math.round(m*ppm);
  ...
  private _dayColumns(ws:Date){
    const cols:any[]=[]; const ppm=Number(this._config.px_per_min)||1.6; const contentPx=Math.round(1440*ppm);
    const today=new Date(); today.setHours(0,0,0,0);
    for(let i=0;i<7;i++){ const d=new Date(ws); d.setDate(d.getDate()+i); const isToday=d.getTime()===today.getTime();
      const labelTxt=d.toLocaleDateString(this._config.locale || "en", { weekday:"short", day:"2-digit", month:"short" });
      const day=this._days[i];
      const allday = this._config.show_all_day ? html`<div class="allday">${(day?.allDay||[]).map(n=> html`<div class="pill" @click=${()=>this._openDetails(n)}>${n.summary}</div>`)}</div>` : nothing;
      const laneCount=day?.laneCount||1;
      const timedBlocks=(day?.timed||[]).map(ev=>{ const topPx=Math.round(ev.top*ppm); const hPx=Math.max(2, Math.round(ev.height*ppm));
        const widthPct=100/laneCount; const leftPct=ev.lane*widthPct; const color=ev.n.color; const text=textOn(color); const bg=softBg(color,0.55);
        return html`<div class="event" title=${ev.n.summary} style=${`top:${topPx}px;height:${hPx-2}px;left:${leftPct}%;width:calc(${widthPct}% - 4px);background:${bg};border:1px solid ${color};color:${text}`} @click=${()=>this._openDetails(ev.n)}><div class="title">${ev.n.summary}</div></div>`;
      });
      const nowLine = (this._config.show_now_indicator && this._weekOffset===0) ? this._nowLineForDay(i) : nothing;
      cols.push(html`<div class=${isToday? "col today":"col"} style=${`grid-column:${2+i}/${3+i}`}>
  ...
  private _nowLineForDay(i:number){ const now=new Date(); const start=new Date(this._weekStart); start.setDate(start.getDate()+i); const end=addMin(start,24*60); if(now<start||now>end) return nothing; const ppm=Number(this._config.px_per_min)||1.6; const mins=(now.getTime()-start.getTime())/60000; const yPx=Math.round(mins*ppm); return html`<div class="now" style=${`top:${yPx}px`}></div>`; }
  private _openDetails(n:NormalizedEvent){ const msg = `${n.summary}\n${this._fmtRange(n.s,n.e)}${n.location?`\n${n.location}`:""}${n.description?`\n\n${n.description}`:""}`; window.alert(msg); }
  private _anyEvents(){ return (this._days||[]).some(d => (d.allDay?.length||0) + (d.timed?.length||0) > 0); }
}

declare global { interface Window { customCards?: any[] } }
window.customCards = window.customCards || [];
if(!window.customCards.find(c=>c.type===TAG)){
  window.customCards.push({ type: TAG, name: "Multi-Calendar Grid Card", description: "7-day time-grid overlay for multiple calendar entities (Lit+TS)", preview: false });
}
